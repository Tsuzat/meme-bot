...
def get_request(sub_reddit="dankmemes", topic="hot", limit="50"):
    ...
    req = requests.get(
        f'https://oauth.reddit.com/r/{sub_reddit}/{topic}', headers=headers, params={'limit': limit})
    ...
    if req.ok:  # request ok then return it
        ...
        # one can use any User-Agent name, but I'm using just memebot
...
def send_meme(subreddit="dankmemes", method="hot"):
    try:  # try to get data
        req = get_request(sub_reddit=subreddit or "dankmemes",
                          topic=method or "hot")
        json_data = req.json()
